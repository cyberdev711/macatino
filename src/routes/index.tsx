...
function Index() {
  return <main id="top">
    ...
    <section id="blog" className="section blog"><Eyebrow>Our Blog</Eyebrow><h2>Stay Informed with Logistics Trends and Insights</h2><div className="blog-grid">{[[blogOne.url,"common-logistics-challenges-and-solutions-for-growing-companies","May 26, 2026","Common Logistics Challenges and Solutions for Growing Companies"],[blogTwo.url,"best-logistics-management-techniques-for-small-business-operations","May 24, 2026","Best Logistics Management Techniques for Small Business Operations"]].map(x=><Link to="/blog/$slug" params={{slug:x[1]}} key={x[3]}><article><img src={x[0]} alt={x[3]}/><span>{x[2]}</span><h3>{x[3]}</h3></article></Link>)}</div></section>
